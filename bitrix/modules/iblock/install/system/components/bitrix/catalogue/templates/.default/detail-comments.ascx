...
<script runat="server">
    protected override void OnInit(EventArgs e)
    {
        ...
        if (!(parentComponent != null 
			&& parentComponent.AllowComments 
			&& parentComponent.Parameters.GetInt("CommentsForumId", 0) > 0
			&& (blockId = parentComponent.IBlockId) > 0 
			&& (elementId = parentComponent.ComponentCache.GetInt("ElementId", 0)) > 0 || 
			!String.IsNullOrEmpty(parentComponent.ComponentCache.GetString("ElementCode", ""))))
        {
		...
		BXIBlockElementCollection elementCol = null;
		if(elementId > 0)
			elementCol = BXIBlockElement.GetList(
			...
			elementCol = BXIBlockElement.GetList(
				new BXFilter(new BXFilterItem(BXIBlockElement.Fields.Code, BXSqlFilterOperators.Equal, 
				parentComponent.ComponentCache.GetString("ElementCode", ""))),
				null,
				new BXSelectAdd(BXIBlockElement.Fields.CustomFields[blockId]),
				null,
				BXTextEncoder.EmptyTextEncoder
