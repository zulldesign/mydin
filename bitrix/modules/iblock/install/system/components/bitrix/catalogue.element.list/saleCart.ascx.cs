...
{
    public partial class CatalogueElementListSaleCart : UserControl, CatalogueElementListComponent.ISaleCart
    {
		private int UserId
		{
			get
        ...
        public string GetCartGuid(HttpRequest request)
        {
            ...
            if(cookie != null) return cartGuid = cookie.Value;

			if ( UserId > 0 )
			{
				var cart = BXSaleCart.GetByUserId(UserId);
				if (cart != null)
				    return cartGuid = cart.UniqueId;
			}
			return string.Empty;
        }
		...
		public CatalogueElementListComponent.SaleCartItem[] GetSaleCartItems(HttpRequest request, HttpResponse response)
		{
			string guid = GetCartGuid(request),
				newGuid;

			var srcItems = BXSaleCartItemManager.GetSaleCartItems(guid, UserId, BXSite.Current.Id, out newGuid);
			var dstItems = new CatalogueElementListComponent.SaleCartItem[srcItems.Count];
			for(int i = 0; i < srcItems.Count; i++)
			{
				...
				var dstItem = new CatalogueElementListComponent.SaleCartItem();
