...
<div class="blog-comment-form" id="<%= PostFormContainerClientID %>" style="display:none;" >
<div class="blog-comment-form-box">
...
<div class="blog-comment-preview" id="<%= PostPreviewContainerClientID %>" >
    <div class="blog-comment">
	    <div class="blog-comment-cont">
			<div class="blog-comment-cont-white">
				<div class="blog-comment-content"><% PreparePreview(Content.Text, CurrentWriter); %></div>
			</div>
		</div>
    </div>
...
<div class="blog-comment-fields">	
	<% if (!Component.IsAuthentificated) { %>
		...
		<div class="blog-comment-field-user">
			...
			<div class="blog-comment-field-user-sep">&nbsp</div>
			<% if (Component.ShowGuestEmail) { %>
			<div class="blog-comment-field blog-comment-field-email">
				<div class="blog-comment-field-text">
					<label for="<%= GuestEmail.ClientID %>">E-mail<% if (Component.RequireGuestEmail) { %><span class="blog-required-field">*</span><% } %></label>
				</div>
	...
	<% } %>
		...
		<div class="blog-comment-field blog-comment-field-captcha">
			<div class="blog-comment-field-captcha-label">
				...
				<label class="blog-comment-field-caption" for="<%= GuestCaptcha.ClientID %>"><%= GetMessage("Label.GuestCaptcha") %><span class="blog-required-field">*</span></label>
				<br />
		...
		<% if (Component.CanApprove) { %>
		<div class="blog-comment-field blog-comment-field-settings">
			<div class="blog-comment-field-setting"><asp:CheckBox runat="server" ID="Hide" TabIndex="8" Text="<%$ LocRaw:CheckBoxTitle.HidePostAfterCreation %>" />
    ...
    Bitrix.ForumCommentBlockTemplate.prototype = {
        ...
        quote: function(containerId, contentContainerId, authorName, enableBBCode, parentPostId) {
            ...
            if(selectedText.length == 0){
		        ...
		        // Quote & Code
		        messageContainerHtml = messageContainerHtml.replace(/\<table\s*class\s*\=\s*(\"|\')?blog-quote(\"|\')?([^>]*)\>\s*\<tbody\>\s*(\<tr\>\s*\<th\>\s*([^<]*)\s*\<\/th\>\s*\<\/tr\>\s*)?\s*\<tr\>\s*\<td\>/gi, "\001");
		        ...
		        messageContainerHtml = messageContainerHtml.replace(/\<div[^\>]*class\s*=\s*(?:\"|\')?blog-code-box(?:\"|\')?[^\>]*\>([\w\W]*?)\<\/div\>/ig, this._prepareCodeBlockForQuote);				
