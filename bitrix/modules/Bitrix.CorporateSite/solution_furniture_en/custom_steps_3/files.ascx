...
<script runat="server">
	...
	protected override BXWizardResult OnActionNext(Bitrix.DataTypes.BXCommonBag parameters)
	{
		...
		var rules = new List<BXInstallHelperFileRule>(new []
		{
			...
			new BXInstallHelperFileRule { Regex = @"^src/company\.jpg$", Include = true },
			new BXInstallHelperFileRule { Regex = @"^src/people/", Include = true },
			new BXInstallHelperFileRule { Regex = @"^sef\.corporate\.config$" },
		...
		UI.SetProgressBarValue("Installer.ProgressBar", "Bitrix.CorporateSite", 1);
		...
		else if (!File.Exists(pathToBannerHtml))
		{
			...
			File.Copy(BXPath.MapPath(BXPath.Combine(solutionPath, "public/src/banner.jpg")), Path.Combine(assets, "banner.jpg"), true);
			File.WriteAllText(pathToBannerHtml, "<img src=\"" + site.DirectoryAbsolutePath + "src/banner.jpg\"/>");
		}
