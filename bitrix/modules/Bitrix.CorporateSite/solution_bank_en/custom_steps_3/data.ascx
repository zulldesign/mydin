...
<script runat="server">
	...
	{
		if (!WizardContext.State.Get<BXParamsBag<object>>("Bitrix.BankSite.Settings").GetBool("InstallDemoData"))
			return Result.Next();
		...
		var importer = new IBlockXmlImporter(siteId, "Bitrix.BankSite", "iblock");
		importer.LoadInfoBlocks(BXPath.MapPath(BXPath.Combine(solutionPath, "data/iblock_data.xml")));
		UI.SetProgressBarValue("Installer.ProgressBar", "Bitrix.BankSite", 3);
		return Result.Next();
	}
</script>
Download Demo Data
