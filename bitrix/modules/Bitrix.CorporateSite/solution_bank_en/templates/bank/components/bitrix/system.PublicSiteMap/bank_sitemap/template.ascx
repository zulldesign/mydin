...
<% 
	if (Component.RootNode != null)
	{
		...
		int perc = (int)100 / (Component.NumberOfColumns==0 ? 1 : Component.NumberOfColumns);
		int counter = 0;
		%>
<table class="map-columns" cellpadding="0">
<tr>
	<td width="<%=perc %>%">
	...
	foreach (Bitrix.BXPublicMenuItem item in Component.RootNode)
	{
		...
		{
			string ss = RecurseTree(item.Children, 1);
			%>
			<li><a href="<%= Encode(item.Href) %>"><%= Encode(item.Title)%></a>
			<%= ss %>
			<%
			counter++;
		}
...
<script runat="server" type="text/C#">
	protected string RecurseTree(BXPublicMenuItemCollection collection, int level)
	{
		...
		if (collection != null)
		{
			...
			foreach (Bitrix.BXPublicMenuItem item in collection)
			{
				if (item.IsAccessible)
				{
					...
					sb.Append(RecurseTree(item.Children, level + 1));
				}
			}
			sb.Append("</ul>");
		}
