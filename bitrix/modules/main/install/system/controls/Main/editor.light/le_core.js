function JCLightHTMLEditor(arConfig) {
    this.IsReady = false;
    this.Init(arConfig);
}
...
Init: function(arConfig)
{
	...
	// View mode switch
	if (this.arConfig.viewModeInputName)
	{
		var scs = BX.create("INPUT", { props: { type: 'hidden', name: this.arConfig.viewModeInputName }});
		this.pViewModeInput = this.pEditCont.appendChild(scs);
	}
	...
	var taProps = {className: 'lha-textarea', rows: 25, id: this.arConfig.inputId};
	if (this.arConfig.tabIndex)
		taProps.tabIndex = this.arConfig.tabIndex;
	this.pTextarea = this.pSourceDiv.appendChild(BX.create("TEXTAREA", {props: taProps }));
	...
	if (this.arConfig.bSetDefaultCodeView && this.sourseBut)
	{
		...
		this.SetContent(this.arConfig.content);
	}
	...
	BX.ready(function(){
        ...
        _this.IsReady = true;
        BX.onCustomEvent(window, 'LHE_OnReady', [this]);
	});
}, 
...
CreateFrame: function()
{
	...
	var iFrameProps = { id: 'LHE_iframe_' + this.id, className: 'lha-iframe', src: "javascript:void(0)", frameborder: 0};
	if (this.arConfig.tabIndex)
		iFrameProps.tabIndex = this.arConfig.tabIndex;
	this.iFrame = this.pEditCont.appendChild(BX.create("IFRAME", {props: iFrameProps}));
...
OnKeyDown: function(e)
{
	...
	if (e.shiftKey && e.keyCode == 46 /* Del*/)
	{
...
SetView: function(sType)
{
	...
	if (this.pViewModeInput)
		this.pViewModeInput.value = sType;
...
SetEditorContent: function(sContent)
{
	...
	this.pEditorDocument.write('<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01//EN" "http://www.w3.org/TR/html4/DTD/strict.dtd">');
	this.pEditorDocument.write('<html><head>');
	if (this.arConfig.arUserStyles)
	{
		var len = this.arConfig.arUserStyles.length;
		for (var i = 0; i < len; i++)
			this.pEditorDocument.write('<link rel="stylesheet" type="text/css" href="' + Bitrix.HttpUtility.htmlEncode(this.arConfig.arUserStyles[i]) + '" />');
	}
	this.pEditorDocument.write('</head><body style="border: 0">' + sContent + '</body></html>');
	...
	switch(pNode.nodeType)
	{
		...
		case 1:
			...
			oNode.preformat =
				...
				&& (
					(pParentNode && pParentNode.preformat)
					|| (oNode.text == 'pre' && (!pNode.style.whiteSpace || pNode.style.whiteSpace == '' || pNode.style.whiteSpace == 'inherit'))
		...
		case 3:
			...
			//if(!pParentNode || (pParentNode.text != 'pre' && pParentNode.text != 'code'))
			if(!pParentNode || (!pParentNode.preformat && pParentNode.arAttributes['class'] != 'lhe-code'))
			{
				res = res.replace(/\n+/g, ' ');
				res = res.replace(/ +/g, ' ');
			}
...
GetNodeInnerHTML: function(pNode)
{
	var res = '';
...
GetNodeHTMLLeft: function(pNode)
{
	...
	{
		if (pNode.text.toUpperCase() == 'BR' && pNode.oParent && pNode.oParent.preformat)
			return '\r\n';
...
GetSelectionObject: function()
{
	...
	if (typeof this.pEditorWindow.getSelection == 'function') // W3C
	{
	...
	else if (this.pEditorDocument.selection) // IE
	{
...
DetachOuterElement: function(pElement, sBeforeHtml, sAfterHtml)
{
	var parent = pElement.parentNode;
	var sibling = pElement.nextSibling;
	parent.removeChild(pElement);

	if (typeof sBeforeHtml == 'string' && sBeforeHtml != '')
	{
		var dummy = this.pEditorDocument.createElement('DIV');
		dummy.innerHTML = sBeforeHtml;
	...
	{
		var node = pElement.childNodes[0];
	...
	if (typeof sAfterHtml == 'string' && sAfterHtml != '')
	{
		var dummy = this.pEditorDocument.createElement('DIV');
		dummy.innerHTML = sAfterHtml;
