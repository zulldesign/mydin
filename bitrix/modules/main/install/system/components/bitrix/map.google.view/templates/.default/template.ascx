<%@ Reference VirtualPath="~/bitrix/components/bitrix/map.google.view/component.ascx" %>
<%@ Import Namespace="System.Globalization" %>
<%@ Import Namespace="Bitrix.Services" %>
<%@ Control Language="C#" AutoEventWireup="false" Inherits="MapGoogleViewComponentTemplate" %>
<script runat="server">    
    protected override void OnPreRender(EventArgs e)
    {
        ...
        Bitrix.UI.BXPage.RegisterScriptInclude(string.Concat("~/bitrix/controls/Main/map.google/messages.js.aspx?lang=", HttpUtility.UrlEncode(BXLoc.CurrentLocale)));
        ...
        ScriptManager.RegisterStartupScript(
            ...
            string.Format(@"Bitrix.EventUtility.addEventListener(window, ""load"", function(){{ Bitrix.GoogleMapsManager.createEntity(Bitrix.GoogleMapsData.fromObject({{initialState:{0}, id:""{1}"", mapControlTypes:""{2}"", mapOptions:""{3}"", draggableCursor:""{4}"", draggingCursor:""{5}"" }}), document.getElementById(""{6}"")).construct(); }});",
