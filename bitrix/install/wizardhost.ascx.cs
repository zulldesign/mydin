...
{
	public partial class WizardHost : BXControl
	{
		...
		protected string wizardSessionFile;
		...
		protected override void OnInit(EventArgs e)
		{
			...
			wizardSessionFile = HostingEnvironment.MapPath(Page.AppRelativeTemplateSourceDirectory + "installer.session");
			...
			if (File.Exists(wizardSessionFile))
				LoadSession();
		...
		private void DoProcess(bool initWizard)
		{
			...
			if (Request.RequestType == "POST" && !getBehaviour)
			{
			...
			if (!string.IsNullOrEmpty(nextStep) && action == null)
			{
				step = nextStep;
				action = nextAction ?? "";
			}
		...
		private void ProcessResult(string step, string action, BXWizardResult result)
		{
					...
					case BXWizardResultType.Action:
						{
							...
							nextStep = !string.IsNullOrEmpty(ra.Step) ? ra.Step : step;
							nextAction = ra.Action ?? "";
						}
						break;
			...
			if (!string.IsNullOrEmpty(nextStep))
			{
				...
				if (Request.QueryString["inplace"] != null)
				{
					url += (url.IndexOf('?') == -1) ? "?inplace=" : "&inplace=";
					url += "&random=" + new Random().Next().ToString();
				}
		...
		{
			WizardHostFinishEventArgs args = new WizardHostFinishEventArgs(context.State);
			try
			{
				if (Finish != null)
					Finish(this, args);
		...
		private void LoadSession()
		{
			...
			using (XmlReader xml = XmlReader.Create(wizardSessionFile))
			...
			nextStep = innerState.GetString("nextStep");
			nextAction = innerState.GetString("nextAction");
		...
		{
			innerState.Remove("nextStep");
			innerState.Remove("nextAction");
			...
			if (!string.IsNullOrEmpty(nextStep))
			{
				innerState["nextStep"] = nextStep;
				innerState["nextAction"] = nextAction;
			}
			...
			using (XmlWriter xml = XmlWriter.Create(wizardSessionFile))
		...
		private void CreateSession()
		{
			...
			if (InitState != null)
				InitState(this, new WizardHostInitStateEventArgs(context.State));
		}
...
><span>{0}</span></a>",
		...
		class InnerContext : BXWizardContext
		{
			...
			public InnerContext(WizardHost host, BXParamsBag<object> state)
			{
				this.host = host;
				this.state = state;
	...
	public class WizardHostFinishEventArgs : EventArgs
	{
		internal WizardHostFinishEventArgs(BXCommonBag state)
		{
			this.state = state;
		}
	...
	public class WizardHostInitStateEventArgs : EventArgs
	{
		internal WizardHostInitStateEventArgs(BXCommonBag state)
		{
			this.state = state;
		}
