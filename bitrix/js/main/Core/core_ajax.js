...
BX.userOptions.send = function(callback)
{
	...
	if (sParam != '')
	{
		...
		BX.ajax({
			...
			'processData': false,
			'cache': false,
			'url': jsUtils.Path.ToAbsolute('~/bitrix/handlers/Main/UserOptions.ashx') + '?' +sParam + '&' + dotNetVars.securityTokenPair,
			'onsuccess': callback
		});
	}
}
...
{
	BX.ajax.get(jsUtils.Path.ToAbsolute('~/bitrix/handlers/Main/UserOptions.ashx') + '?action=delete&c=' + sCategory + '&n=' + sName + (bCommon == true ? '&common=Y' : '') + '&' + dotNetVars.securityTokenPair, callback);
}
