...
function JCAdminMenu(sOpenedSections, clientId)
{
	var _this = this;
	this.sMenuSelected='';
	this.x = 0;
	...
	this.verSplitter = new JCSplitter({
		...
		messHide:dotNetVars.messHideMenu, messShow:dotNetVars.messShowMenu,
	...
	this.horSplitter = new JCSplitter({
		...
		messHide:dotNetVars.messHideButtons, messShow:dotNetVars.messShowButtons,
	...
	this.ToggleMenu = function(menu_id, menu_text)
	{
		...
		document.getElementById(this.controlClientId + '_btn_'+this.sMenuSelected).className = 'button';
		document.getElementById(this.controlClientId + '_smbtn_'+this.sMenuSelected).className = 'smbutton';
		document.getElementById(this.controlClientId + '_btn_'+menu_id).className = 'button buttonsel';
		document.getElementById(this.controlClientId + '_smbtn_'+menu_id).className = 'smbutton smbuttonsel';
	...
	this.ToggleDynSection = function(cell, module_id, div_id, level)
	{
		...
		if(div.innerHTML == '')
		{
			div.innerHTML = MenuText(dotNetVars.messMenuLoading);

			this.request.Action = function(result)
			{
				result = jsUtils.trim(result);
				div.innerHTML = (result != ''? result : MenuText(dotNetVars.messNoData));
			}
			this.request.Send(jsUtils.Path.ToAbsolute('~/bitrix/handlers/main/GetMenu.ashx') + '?lang='+dotNetVars.LANGUAGE_ID+'&admin_mnu_module_id='+module_id+'&admin_mnu_menu_id='+jsUtils.urlencode(div_id));
		}
...
function JCAdminFilter(filter_id, aRows)
{
	...
	this.InitFilter = function(oVisRows)
	{
		...
		for(i=0; i<n; i++)
		{
			...
			if(i>0)
			{
				...
				td.innerHTML = '<a href="javascript:void(0)" onclick="this.blur();'+this.filter_id+'.ToggleFilterRow(\''+row.id+'\');" hidefocus="true" title="'+dotNetVars.messFilterLess+'" class="context-button icon" id="filterless"></a>';
			}
		}
	...
	this.SetActive = function(on)
	{
		...
		div.title = (on? dotNetVars.messFilterActive:dotNetVars.messFilterInactive);
	}
			...
			if(i>=2)
			{
				...
				if(row.oncontextmenu)
				{
					jsUtils.addEvent(row, "contextmenu",
						...
						{
							if(!e) e = window.event;
							if(!dotNetVars.opt_context_ctrl && e.ctrlKey || dotNetVars.opt_context_ctrl && !e.ctrlKey)
								return;
...
function ShowWaitWindow()
{
	...
	div.innerHTML = dotNetVars.messLoading;
...
function TabControl(name, unique_name, aTabs)
{
	...
	{
		var div = document.getElementById(this.name+'_content_'+tab_id);
		if(div.style.display != 'none')
			return;

		for (var i = 0, cnt = this.aTabs.length; i < cnt; i++)
		{
			var tab = document.getElementById(this.name+'_content_'+this.aTabs[i]["DIV"])
	...
	this.ShowTab = function(tab_id, on)
	{
		var sel = (on? '-selected':'');
		try{
		document.getElementById(this.name+'_tab_cont_'+tab_id).className = 'tab-container'+sel;
		document.getElementById(this.name+'_tab_left_'+tab_id).className = 'tab-left'+sel;
		document.getElementById(this.name+'_tab_'+tab_id).className = 'tab'+sel;
		if(tab_id != this.aTabs[this.aTabs.length-1]["DIV"])
			document.getElementById(this.name+'_tab_right_'+tab_id).className = 'tab-right'+sel;
		else
			document.getElementById(this.name+'_tab_right_'+tab_id).className = 'tab-right-last'+sel;
		}catch(e){}
	}

	this.HoverTab = function(tab_id, on)
	{
		var tab = document.getElementById(this.name+'_tab_'+tab_id);
		if(tab.className == 'tab-selected')
			return;

		document.getElementById(this.name+'_tab_left_'+tab_id).className = (on? 'tab-left-hover':'tab-left');
		tab.className = (on? 'tab-hover':'tab');
		var tab_right = document.getElementById(this.name+'_tab_right_'+tab_id);
		if(tab_id != this.aTabs[this.aTabs.length-1]["DIV"])
	...
	this.InitEditTables = function()
	{
		...
		{
			var div = document.getElementById(this.name+'_content_'+this.aTabs[tab]["DIV"]);
	...
	{
		var tab = document.getElementById(this.name+'_tab_cont_'+tab_id);
		if(disabled)
		{
	...
	this.Destroy = function()
	{
		...
		for(var i = 0, cnt = this.aTabs.length; i < cnt; i++)
		{
			var tab = document.getElementById(this.name+'_tab_cont_'+this.aTabs[i]["DIV"]);
			if (!tab)
				continue;
			tab.onclick = null;
			tab.onmouseover = null;
	...
	this.ToggleTabs = function()
	{
		...
		if(!this.bExpandTabs)
		{
			...
			var div = document.getElementById(this.name+'_content_'+this.aTabs[0]["DIV"]);
			div.style.display = 'block';
		}
...
//			}
//			img.src = '/bitrix/themes/'+dotNetVars.ADMIN_THEME_ID+'/images/icon_warn.gif';
...
//		request.Post(jsUtils.Path.ToAbsolute('~/bitrix/handlers/Main/UserOptions.ashx') +'?lang='+dotNetVars.LANGUAGE_ID+ '&' +dotNetVars.securityTokenPair, sParam);
//	}
...
//		request.Send(jsUtils.Path.ToAbsolute('~/bitrix/handlers/Main/UserOptions.ashx') + '?lang=' + dotNetVars.LANGUAGE_ID + sParam + '&' + dotNetVars.securityTokenPair);
//	}
...
function ViewTabControl(name, aTabs)
{
	var _this = this;
	this.aTabs = aTabs;
	this.name = name;
	...
	this.SelectTab = function(tab_id)
	{
		...
		for(var i in this.aTabs)
		{
			var tab_div = document.getElementById(this.name + '_content_' + this.aTabs[i]["DIV"]);
			if(tab_div.style.display != 'none')
			{
				var tab = document.getElementById(this.name + '_view_tab_' + this.aTabs[i]["DIV"]);
		...
		var active_tab = document.getElementById(this.name + '_view_tab_' + tab_id);
	...
	{
		var tab_div = document.getElementById(this.name + '_view_tab_' + tab["DIV"]);
	...
	{
		var container = jsUtils.FindParentObject(document.getElementById(_this.name + '_view_tab_' + _this.aTabs[0]["DIV"]), "div");
		var aPos = [0];
		var selectedIndex = -1;
		var prevTop = -1;
		var last;
	...
	this.Init = function()
	{
		...
		for(var i in this.aTabs)
		{
			var div = document.getElementById(this.name + '_content_' + this.aTabs[i]["DIV"]);
			if(div.style.display != 'none')
			{
				this.ReplaceAnchor(this.aTabs[i]);
				this.ToggleDelimiter(document.getElementById(this.name + '_view_tab_' + this.aTabs[i]["DIV"]), false);
				break;
			}
		}
...
var jsAdminChain =
{
	...
	AddItems: function(divId)
	{
		...
		main_chain.innerHTML += '<img src="' + jsUtils.Path.ToAbsolute('~/bitrix') + '/themes/'+dotNetVars.ADMIN_THEME_ID+'/images/chain_arrow.gif" alt="" border="0" class="arrow">';
...
function JCUserOptions()
{
	...
	this.SaveOption = function(sCategory, sName, sValName, sVal, bCommon, millisecondsToSend) {
	    if (!this.options)
	    ...
	    if (sParam != '')
	        document.cookie = dotNetVars.cookiePrefix + "_LAST_SETTINGS=" + sParam + "; expires=Thu, 31 Dec 2020 23:59:59 GMT; path=/;";

	    if (!this.bSend) {
	        this.bSend = true;
	        var time;
	        
	        if (millisecondsToSend) time = millisecondsToSend
	        else time = 5000;
	        setTimeout(function() { _this.SendData(null) }, time);
	    }
	}
	...
	this.SendData = function(callback)
	{
		...
		if(sParam != '')
		{
			document.cookie = dotNetVars.cookiePrefix+"_LAST_SETTINGS=; path=/;";
			_this.request.Action = callback;
			_this.request.Send(jsUtils.Path.ToAbsolute('~/bitrix/handlers/Main/UserOptions.ashx') + '?' + sParam + '&' + dotNetVars.securityTokenPair);
		}
	}
	...
	{
		_this.request.Action = callback;
		_this.request.Send(jsUtils.Path.ToAbsolute('~/bitrix/handlers/Main/UserOptions.ashx') + '?action=delete&c=' + sCategory + '&n=' + sName + (bCommon == true ? '&common=Y' : '') + '&' + dotNetVars.securityTokenPair);
	}
}
...
function JCPanel()
{
	...
	this.FixPanel = function() {
	    ...
	    var bIE = jsUtils.IsIEPre8();
	    if (bFixed) {
	        a.title = dotNetVars.messPanelFixOn;
	    ...
	    jsUserOptions.SaveOption('admin_panel', 'settings', 'fix', (bFixed ? 'off' : 'on'),null, 1);
	}
	...
	this.ShowOn = function()
	{
		...
		a.title = dotNetVars.messPanelFixOff;
...
//				document.body.style.backgroundImage = "url(javascript:false;)";
				document.body.style.backgroundImage = "url(" + jsUtils.Path.ToAbsolute("~/bitrix/images/1.gif") + ")";
	...
	this.DisplayPanel = function(el)
	{
		...
		if(div.style.display == 'none')
		{
			...
			el.title = dotNetVars.messPanelCollapse;
			jsUserOptions.SaveOption('admin_panel', 'settings', 'collapsed', 'off',null,1);
		}
		else
		{
			div.style.display = 'none';
			el.className = 'splitterknob splitterknobdown';
			el.title = dotNetVars.messPanelExpand;
			jsUserOptions.SaveOption('admin_panel', 'settings', 'collapsed', 'on',null,1);
		}
		var panel = document.getElementById('bx_top_panel_container');
		var backDiv = document.getElementById('bx_top_panel_back');
		backDiv.style.height = panel.offsetHeight+'px';
...
{
    //---
    var loadingMsg = "Загрузка...";
    //var lang = "ru";
    //---
	this.iframe = null;
	var _this = this;

	//this.messLoading = dotNetVars.messLoading;
    this.messLoading = loadingMsg;
	...
	this.Open = function(wizardName, sessid)
	{
		...
		div.innerHTML =
			...
			'</table>'+
			'</div>'+
			'<div class="wizard" style="height:387px;"><iframe class="content" style="background-color: transparent;" allowtransparency="true" scrolling="no" id="wizard_iframe" width="100%" src="/bitrix/admin/install/default.aspx" height="100%" frameborder="0">'+
			''+
			'</iframe></div>';
...
function JCStartMenu()
{
	...
	this.ShowStartMenu = function(button, back_url)
	{
		...
		if(!menuStart || !menuStart.menuItems)
		{
			...
			menuStart.ShowMenu(button, [{
				'TEXT':dotNetVars.messMenuLoading,
				'TITLE':dotNetVars.messMenuLoadingTitle,
				'ICONCLASS':'loading',
				'AUTOHIDE': false}], jsPanel.IsFixed(), dPos);
				
			request.Send(BX.admin.startMenuSettings.handlerUrl + '?' + dotNetVars.securityTokenPair);
		}
		else
		{
	...
	this.PreloadMenu = function(back_url)
	{
		if(!menuStart)
		{
			...
			menuStart = new PopupMenu('panel_start_menu');
			menuStart.Create(1100);
			request.Send(BX.admin.startMenuSettings.handlerUrl + '?lang=' + dotNetVars.LANGUAGE_ID + (back_url ? '&back_url_pub=' + encodeURIComponent(back_url) : '') + '&' + dotNetVars.securityTokenPair);
		}
	}
	...
	this.OpenDynMenu = function(menu, module_id, items_id, back_url)
	{
		...
		request.Send(BX.admin.startMenuSettings.handlerUrl + '?mode=dynamic&lang=' + dotNetVars.LANGUAGE_ID + '&admin_mnu_module_id=' + jsUtils.urlencode(module_id) + '&admin_mnu_menu_id=' + jsUtils.urlencode(items_id) + '&' + dotNetVars.securityTokenPair);
	}
	...
	this.OpenURL = function(item, arguments, url, back_url)
	{
		...
		{
			request.Action = function(result){}
			request.Send(BX.admin.startMenuSettings.handlerUrl + '?mode=save_recent&url=' + encodeURIComponent(url) + '&text=' + jsUtils.urlencode(itemInfo['TEXT']) + '&title=' + jsUtils.urlencode(itemInfo['TITLE']) + '&icon=' + itemInfo['ICON'] + '&' + dotNetVars.securityTokenPair);
		}
		if(back_url)
...
function JCHttpRequest2()
{
    // Each server response must contain:
    // <%--ajax id="1" handler="def" --%>
    // id = unique id of response
    // handler = to what handler pass the response

    // Default handler contains 'def.' like: def.1
	this.ActionHandler = {};
	// Handlers history count (keeps last response id for each handler)
	this.HandlerHistory = {};
	// Stack request on id to pass to what handler, ex.: 314 request => pass to def.5 handler
	this.RequestQueue = [];
	// Current header of response
	this.CurrentHeader = null;
	//
	this._freeuid = 0;
	this._headerRX = new RegExp('<%--ajax[\\x20]?([^>]+)[\\x20]?--%>', 'i');
	this._paramRX = new RegExp('([\\w]+)="([^"]+)"', 'ig');
	this._paramDRX = new RegExp('<%--"([^"]*)":"([^"]*)"--%>', "i");

    this.ActionHandler['def.load.scripts'] = function(s)
    {
        try
        {
            var code = [];
			...
			for(var i in code)
				if(code[i] != '')
					...
					{
					    alert('Loaded script failed: '+e);
					}
		}
		catch (e) { alert('Unknown error executing answer.'); }
    }

    this.QueueRequest = function(delegate)
    {
        this.RequestQueue[this._freeuid + 1] = delegate;
    }

    this.TrySetHandler = function(name, delegate)
    {
        if (!this.ActionHandler[name])
        {
            this.ActionHandler[name] = delegate;
            this.HandlerHistory[name] = 0;
        }
    }

    this.CallHandler = function(handler, s)
    {
        if (this.ActionHandler[handler] && typeof this.ActionHandler[handler] == 'function')
            this.ActionHandler[handler](s);
    }

    this.CallDefaultHandlers = function(s)
    {
        for (var key in this.ActionHandler)
            if (key.indexOf('def.')>-1)
                if (this.ActionHandler[key])
                    this.ActionHandler[key](s);
    }
	...
	this._ParseResponce = function(s)
	{
		this.CurrentHeader = {'id':null, 'handler':null};
		var m1 = this._headerRX.exec(s), m2;
		if (m1 && m1[1])
		{
			while (m2 = this._paramRX.exec(m1[1]))
			    this.CurrentHeader[m2[1]] = m2[2];
		}
	    m1 = m2 = null;
	    return this.CurrentHeader;
	}

	this.ProcessResponceParams = function(s, header)
	{
		var param, ss = s;
		while (param = this._paramDRX.exec(ss))
		{
			header[param[1]] = param[2];
			ss = ss.replace(param[0], '');
		}
		s = ss;
		return s;
	}
	...
	this._SetHandler = function(httpRequest)
	{
		...
		httpRequest.onreadystatechange = function()
		{
			if(httpRequest.readyState == 4)
			{
				try
				{
					var validcall = true, iid;
					var s = httpRequest.responseText;
					var header = _this._ParseResponce(s);
					s = _this.ProcessResponceParams(s, header);

					if (header != null)
					{
					    // 0) Erase header
					    s = s.replace(_this._headerRX, '');

					    // 1) If there is no handler and queue request, pass it default handlers
					    if (!header.handler)
					    {
					        _this.CallDefaultHandlers(s);
					    }
					    else
					    {
					        iid = parseInt(header.id);
					        // 1) Check history integration
					        if (iid && header.handler)
					        {
							if (_this.HandlerHistory[header.handler] > 0)
							{
								if (_this.HandlerHistory[header.handler] >= iid)
									validcall = false;
								else
									_this.HandlerHistory[header.handler] = iid;
							}
							else
							{
								_this.HandlerHistory[header.handler] = iid;
					        ...
					        // 2) Try queue handler (For Queue there is no history check = validcall)
					        if (iid && _this.RequestQueue[iid])
					        {
					            _this.RequestQueue[iid](s);
					            _this.RequestQueue[iid] = null;
					        }

					        // 3) Make a call to the handler if call is valid
					        if (validcall && header.handler)
					            _this.CallHandler(header.handler, s);
					    }
					} else _this.CallDefaultHandlers(s);
					this.CurrentHeader = header;
					s = header = validcall = whom = null;
				}
				catch (e)
				{
	...
	this._urlToSever = function(url)
	{
	    if (url.indexOf('?')>-1)
	        return url+'&reqid='+(++this._freeuid);
	    else
	        return url+'?reqid='+(++this._freeuid);
	}
	...
	this.Send = function(url)
	{
		...
		{
			this._SetHandler(httpRequest);
			httpRequest.open("GET", this._urlToSever(url), true);
			return httpRequest.send("");
  		}
	}
	...
	this.Post = function(url, data)
	{
		...
		{
			this._SetHandler(httpRequest);
			httpRequest.open("POST", this._urlToSever(url), true);
			httpRequest.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
			return httpRequest.send(data);
  		}
	}
}
var CHttpRequest2 = new JCHttpRequest2();
...
function CloneTableRow(tableId) {
    var tab = document.getElementById(tableId);
    var len = tab.getElementsByTagName('tr').length;
    var root = tab.getElementsByTagName('tr')[0].parentNode;
    var digs="1234567890";
    var newId,dig;
    ...
    for (i = 0; i < inputs.length; i++) {
            inputs[i].id = inputs[i].id.substring(0, inputs[i].id.lastIndexOf('_')+1 ) + (len+1);
            inputs[i].value = "";
