...
<% if (Results.ContainsKey("ShowAlbumSelect")) {
	   var noAlbums = SelectAlbums.Items.Count == 0;
	   %>
...
<bx:BXValidationSummary ID="errorMessage" runat="server" CssClass="errortext" HeaderText="<%$ Loc:Kernel.Error %>" Visible="true"/>
...
function PackageBeforeUploadLink2(PackageIndex)
{
    PackageBeforeUploadLink(PackageIndex);
}
...
//	
	...
	iu.addEventListener("PackageBeforeUpload", "PackageBeforeUploadLink2");
